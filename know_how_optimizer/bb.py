...
class BB(Optimizer):
    def __init__(self, params, lr=1e-1, steps=400, beta=0.01, min_lr=1e-1, max_lr=10.0, weight_decay=0.):
        assert lr > 0.0, ValueError(
            "Invalid initial learning rate: {}".format(lr))
        assert steps > 0, ValueError("Invalid steps: {}".format(steps))
        assert 0.0 < beta <= 1.0, ValueError(
            "Invalid beta value: {}".format(beta))
        assert min_lr > 0.0, ValueError(
            "Invalid minimal learning rate: {}".format(min_lr))
        assert max_lr > min_lr, ValueError(
            "Invalid maximal learning rate: {}".format(max_lr))
        assert weight_decay >= 0.0, ValueError(
        ...
        assert len(self.param_groups) == 1, ValueError(
            "BB doesn't support per-parameter options (parameter groups)")
    ...
    def step(self, closure=None):
        ...
        for p in self._params:
            ...
            with torch.no_grad():
                self.state[p]['grad_aver'].mul_(
                    1 - group['beta']).add_(group['beta'], d_p)
